...
- [~] **T5-06** FE: 새 탭 "승급" 추가 (BottomTabBar) — **스캐폴드 완료** 2026-04-17
  ...
  - 진행 상황:
    - `app/level-up/page.tsx`: 현재/다음 벨트 + 조건 + 비활성 응시 버튼 스캐폴드
    - `components/BottomTabBar.tsx`: 4번째 탭 "승급" (Award 아이콘) 추가
    - i18n `tabs.levelUp` + `levelUp.*` 추가
  - 남은 숙제 (BE 연동 대기):
    - eligibility API 조회 → 자격 여부 분기
...
## 🟣 트랙 6 — 기록 탭 세션 진척도 ✅ 완료 (2026-04-17)
