...
- [x] **05** 홈 페이지 라우트 생성 (`/`)
- [x] **06** `HomeHeader.tsx` — 앱 이름 "코대헌" + 영문 서브타이틀 / 우측 프로필 원형 아이콘 + @유저ID / 티어값에 따른 테두리 컬러 동적 적용 _(BE 연동: 유저 프로필 API)_
- [x] **07** `TierCard.tsx` — 티어명 / 등급 / XP 현재·최대 / 다음 티어까지 남은 XP 표시 _(BE 연동: XP 데이터 API)_
- [x] **08** `WeeklyStats.tsx` — 대화 수 / 평균 점수 / 연속 학습일(스트릭) 3칸 가로 배열 / 스트릭 빨간색 강조 _(BE 연동: 주간 통계 API)_
- [x] **09** 대화 시작 CTA 버튼 — 클릭 시 `/location` 이동 / scale 눌림 애니메이션
- [x] **10** `BottomTabBar.tsx` — 홈/대화/기록/복습/내정보 5탭 / 아이콘+텍스트 / 활성 탭 주황·비활성 회색 / 현재 경로 감지 자동 표시
...
- [x] **12** 온보딩 페이지 라우트 생성 (`/onboarding`)
- [x] **13** 국적 선택 드롭다운 UI
  - 자주 쓰는 국가 최상단 고정 배치
...
- [x] **19** 완료 시 로컬스토리지에 사용자 프로필 저장
- [x] **20** 2회차 접속 시 온보딩 건너뛰고 홈으로 자동 이동
...
- [x] **31** 페르소나 A/B 카드 표시 — **사용자가 맡을 역할/미션** 선택 (이름 / 직업 / 나이 / 미션 목적)
...
- [x] **35** 말풍선형 채팅 메시지 컴포넌트 (내 메시지 우측 / AI 메시지 좌측 구분)
- [x] **36** 메시지 입력창 + 전송 버튼 UI
- [x] **37** AI 응답 스트리밍 표시 (글자가 순서대로 나타나는 효과)
- [x] **38** 남은 턴 수 표시 UI (예: "남은 대화 5 / 7")
- [x] **39** 대화 종료 → **"결과 확인하기" 버튼** 표시 → 클릭 시 `/result` 이동 (자동 이동 제거)
...
- [x] **45** 대화 로그 다시보기 UI (스크롤 가능한 대화 목록)
- [x] **46** 사용자 발화 중 오답 부분 빨간 음영 표시
- [x] **47** 틀린 단어 목록 + 뜻풀이 표시
- [x] **48** 대화 요약 피드백 텍스트 표시
